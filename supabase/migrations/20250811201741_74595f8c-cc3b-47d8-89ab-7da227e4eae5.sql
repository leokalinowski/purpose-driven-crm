...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "Users can view their own newsletter campaigns" ON public.newsletter_campaigns FOR SELECT USING (auth.uid() = created_by OR public.get_current_user_role() = ''admin'')';
  END IF;
END $$;