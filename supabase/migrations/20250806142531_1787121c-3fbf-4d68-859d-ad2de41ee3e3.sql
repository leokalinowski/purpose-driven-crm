...
-- Add missing columns to existing leads table (user_id is already the primary key)
...
CREATE TABLE public.dtd2_tasks (
  ...
  lead_id BIGINT NOT NULL REFERENCES public."Leads Table [Agent's Name]"(user_id) ON DELETE CASCADE,
...
CREATE TABLE public.transaction_coordination (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  lead_id BIGINT REFERENCES public."Leads Table [Agent's Name]"(user_id),
