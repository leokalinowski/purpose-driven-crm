-- PHASE 2: DATA ACCESS SECURITY IMPROVEMENTS (CORRECTED)
...
-- Add data masking function for sensitive contact data (corrected syntax)
...
BEGIN
  ...
  ELSE
    ...
    IF field_name = 'email' THEN
      RETURN mask_email(field_value);
    ELSIF field_name = 'phone' THEN
      RETURN mask_phone(field_value);
    ELSE
      RETURN '[PROTECTED]';
    END IF;
  END IF;
