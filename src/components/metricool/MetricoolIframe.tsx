...
export function MetricoolIframe({ userId }: MetricoolIframeProps) {
  ...
  useEffect(() => {
    ...
    console.log('[MetricoolIframe] Using wrapper URL for user:', userId, wrapperUrl);
  ...
  useEffect(() => {
    ...
    loadTimeoutRef.current = setTimeout(() => {
      console.log('[MetricoolIframe] Hiding loading spinner after 30 seconds (fallback)');
      setIsLoadingIframe(false);
    }, 30000);
  ...
  const handleIframeError = useCallback(() => {
    ...
    console.warn('[MetricoolIframe] Iframe error event detected, hiding spinner');
    setIsLoadingIframe(false);
  }, []);
