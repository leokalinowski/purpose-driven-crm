...
export function MetricoolDashboard({ userId }: MetricoolIframeProps) {
  ...
  useEffect(() => {
    if (metricoolLink && iframeRef.current) {
      console.log('[MetricoolDashboard] Setting up iframe with URL:', metricoolLink.iframe_url);
      setIsLoadingIframe(true);
      setLoadError(null);
      
      // Set timeout to detect if iframe never loads (30 seconds)
      loadTimeoutRef.current = setTimeout(() => {
        console.warn('[MetricoolDashboard] Iframe load timeout after 30 seconds');
        if (isLoadingIframe) {
          setLoadError('Iframe took too long to load. This may indicate the embed is blocked by browser security policies.');
      ...
      <CardContent>
        ...
        <div className="w-full">
          ...
          <iframe
            ...
            loading="lazy"
            onLoad={() => {
              console.log('[MetricoolDashboard] Approach 1 - Direct iframe loaded successfully');
              setIsLoadingIframe(false);
              setLoadError(null);
              if (loadTimeoutRef.current) {
                clearTimeout(loadTimeoutRef.current);
            ...
            onError={(e) => {
              console.error('[MetricoolDashboard] Approach 1 - Direct iframe error:', e);
