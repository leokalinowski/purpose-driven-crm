...
export default function Pipeline() {
  ...
  const { opportunities, loading, updateStage, refresh } = usePipeline();
  const { filtered, boardStages, pipelineType, setPipelineType, showLost, setShowLost } =
    ...
    return (
      <Layout>
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-center max-w-sm mx-auto p-8 rounded-xl border border-border bg-card">
  ...
  const tabs: { key: Tab; label: string; icon: React.ReactNode }[] = [
    { key: 'today', label: 'Today', icon: <LayoutList className="h-3.5 w-3.5" /> },
      ...
      <Layout>
        <DndProvider backend={HTML5Backend}>
          <div className="space-y-4">
            ...
            {/* Content */}
            {activeTab === 'today' ? (
              <TodayView onOpenDetail={(opp) => setDetailOpp(opp as Opportunity)} />
            ) : (
              <PipelineBoard
